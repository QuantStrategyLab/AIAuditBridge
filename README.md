...
`service/dependency_notification_triage.py` is a pure, fail-closed adapter that classifies structured dependency/engineering-review evidence onto existing `BriefingFinding` / `BriefingAction` lanes (`quiet` / `github_issue` / `telegram`) for later use with `dispatch_briefing_result`. It does not add a second PR reviewer, call GitHub notification APIs, send Telegram/GitHub issues, or invoke models. Quiet-gate evidence that is missing, conflicting, or unknown escalates to `telegram` with `review_required` / `unknown`; ordinary source-code or non-Dependabot review still uses `github_issue`.

`service/dependency_notification_source.py` and `scripts/run_dependency_notification_source_dry_run.py` convert allowlisted open PRs (GitHub REST GET only) into the existing trusted intake schema v1 and reuse the dry-run triage preview. An empty allowlist / org-wide scan is rejected. This path does not send alerts, write issues/comments/merges, change `dependency_audit.yml`, or call models. The manual `dependency_notification_source_dry_run.yml` workflow accepts exactly one `QuantStrategyLab/<name>`: self-repo may use `GITHUB_TOKEN`; cross-repo mints a temporary read-only App token from existing `CROSS_REPO_GITHUB_APP_ID` / `CROSS_REPO_GITHUB_APP_PRIVATE_KEY` into `CODEX_AUDIT_GH_TOKEN` only, fail-closed with no `GITHUB_TOKEN` fallback and no new secrets.
