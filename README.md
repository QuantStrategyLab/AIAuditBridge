...
`service/dependency_notification_source.py` and `scripts/run_dependency_notification_source_dry_run.py` convert allowlisted open PRs (GitHub REST GET only) into the existing trusted intake schema v1 and reuse the dry-run triage preview. An empty allowlist / org-wide scan is rejected. This path does not send alerts, write issues/comments/merges, change `dependency_audit.yml`, or call models. Cross-repo reads need existing `CODEX_AUDIT_GH_TOKEN`/`GH_TOKEN`; this batch adds no new secrets.
