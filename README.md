...
When a human has reviewed Codex blocking findings and accepts the residual risk, add the `review-ack` label (configurable via `pr_review.ack_labels`). Codex still posts findings, but the `review` check no longer fails.

Zero-touch deadlock break: after `pr_review.auto_converge_after` consecutive blocking review rounds on the same PR (default **3**), the check auto-passes while still posting findings. No label and no human reading of findings is required. Set `auto_converge_after` to `0` to disable. Repos that call `AIAuditBridge` `codex_pr_review.yml@main` inherit this automatically.
