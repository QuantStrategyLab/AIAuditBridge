...
When a human has reviewed Codex blocking findings and accepts the residual risk, add the `review-ack` label (configurable via `pr_review.ack_labels`). Codex still posts findings, but the `review` check no longer fails. Repos that call `AIAuditBridge` `codex_pr_review.yml@main` inherit this automatically.
