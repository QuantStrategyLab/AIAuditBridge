...
`service/dependency_notification_source.py` 与 `scripts/run_dependency_notification_source_dry_run.py` 在显式仓库 allowlist 下用 GitHub REST GET 读取受控 open PR，转换为已有 trusted intake schema v1，并复用上述 dry-run 分流预览。默认禁止空 allowlist/全组织扫描；不发送通知、不写 issue/comment/merge、不改 `dependency_audit.yml`、不调用模型。跨仓读取需现有 `CODEX_AUDIT_GH_TOKEN`/`GH_TOKEN`；本批不新增 secret。
