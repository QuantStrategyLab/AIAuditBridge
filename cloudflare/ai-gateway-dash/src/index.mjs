...
function renderQuota(q){setUpdated("quota-updated");const target=document.getElementById("quota"),repos=q.repos||{},summary=q.summary||null;clear(target);if(summary){const list=el("div","quota-list"),details=el("div","");appendRepoQuotaRows(list,repos);append(details,quotaTotalRow("合计估算",summary.combined||{}),quotaUsageRow("API Key 估算",summary.api_key||{}),quotaUsageRow("Codex 估算",summary.codex||{}));if(Object.keys(repos).length)target.appendChild(list);target.appendChild(details);target.appendChild(el("div","subtle","这里是内部用量估算，不等同于 OpenAI/Anthropic/Codex 账户后台实时余额；repo 行按实际限流日预算显示。"));return}if(!Object.keys(repos).length){empty(target,"暂无用量","当前窗口内没有记录到配额消耗。API Key 与 Codex 均无用量记录。");return}const list=el("div","quota-list");appendRepoQuotaRows(list,repos);target.appendChild(list)}
