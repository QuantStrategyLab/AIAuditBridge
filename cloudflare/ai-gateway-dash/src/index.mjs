...
<div class="shell">
  ...
  <main>
    <section class="grid">
      ...
      <article class="card span-6"><div class="card-body"><div class="card-head"><h2>配额消耗</h2><span class="updated" id="quota-updated">更新: —</span></div><div id="quota"></div></div></article>
...
function statusText(st){return st==="healthy"?"健康":st==="degraded"?"降级":st==="unhealthy"?"异常":st||"unknown"}
...
function quotaProgress(title,d){const used=Number(d&&d.total_cost_usd)||0,budget=Number(d&&d.daily_budget)||0,p=budget?used/budget*100:0,c=progressClass(p),item=el("div","quota-item"),meta=el("div",""),bar=el("div","bar"),fill=el("div","bar-fill "+c);fill.style.width=Math.max(0,Math.min(p,100))+"%";append(meta,el("div","quota-name",title),el("div","quota-meta",fmtUSD(used)+" / "+fmtUSD(budget)));append(bar,fill);append(item,meta,bar,el("div","bar-label",Math.round(p)+"%"));return item}
...
function renderQuota(q){setUpdated("quota-updated");const target=document.getElementById("quota"),repos=q.repos||{},summary=q.summary||null;clear(target);if(summary){const list=el("div","quota-list"),details=el("div","");list.appendChild(quotaProgress("共享日预算合计",summary.combined||{}));appendRepoQuotaRows(list,repos);append(details,quotaUsageRow("API Key 估算",summary.api_key||{}),quotaUsageRow("Codex 估算",summary.codex||{}));target.appendChild(list);target.appendChild(details);target.appendChild(el("div","subtle","这里是内部用量估算，不等同于 OpenAI/Anthropic/Codex 账户后台实时余额；repo 行仍按实际限流预算显示。"));return}if(!Object.keys(repos).length){empty(target,"暂无用量","当前窗口内没有记录到配额消耗。API Key 与 Codex 均无用量记录。");return}const list=el("div","quota-list");appendRepoQuotaRows(list,repos);target.appendChild(list)}
