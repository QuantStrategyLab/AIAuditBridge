...
function providerAccountRow(label,d,usageKey){const row=el("div","stat-row"),c=d&&d.costs||{},u=d&&d[usageKey]||{},days=Number(d&&d.window_days)||7,currency=String(c.currency||"usd").toUpperCase(),cost=Number(c.total_cost)||0,textTokens=(Number(u.input_tokens)||0)+(Number(u.output_tokens)||0),audioTokens=(Number(u.input_audio_tokens)||0)+(Number(u.output_audio_tokens)||0),tokens=Number(u.total_tokens)||(textTokens+audioTokens),cached=Number(u.input_cached_tokens)||0,requests=Number(u.num_model_requests)||0,parts=[];if(c.total_cost!==null&&c.total_cost!==undefined)parts.push(days+"d "+(currency==="USD"?fmtUSD(cost):cost.toFixed(2)+" "+currency));if(requests)parts.push(fmtNum(requests)+" req");if(tokens)parts.push(fmtNum(tokens)+" tokens");if(audioTokens)parts.push(fmtNum(audioTokens)+" audio");if(cached)parts.push(fmtNum(cached)+" cached");if(d&&d.usage_surface)parts.push(String(d.usage_surface));if(d&&d.filtered_project_count)parts.push(fmtNum(d.filtered_project_count)+" project");if(d&&d.filtered_api_key_count)parts.push(fmtNum(d.filtered_api_key_count)+" key");if(d&&d.filtered_workspace_count)parts.push(fmtNum(d.filtered_workspace_count)+" workspace");append(row,el("span","stat-label",label),el("span","stat-value info",parts.join(" · ")||"实时可用"));return row}
