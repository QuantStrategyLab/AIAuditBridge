...
新一版 review contract 还会为每个 finding 记录稳定的 `contract_key` 和 `behavior_digest`，并用 `fingerprint_v2` 避免只靠 file/category/severity 产生粗粒度碰撞。状态机按 `initial_review -> remediation(每个 contract 最多 2 轮) -> arbitration_required -> cleared / blocked_human_contract` 收敛，超过上限后禁止继续自动修复。
