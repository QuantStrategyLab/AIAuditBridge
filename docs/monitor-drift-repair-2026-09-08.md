...
### 2026-09-09：浅镜像替代目录缺少目标提交对象

在 AAB `dfccd5e21fa5e5ed0c3d226845ee988bdbab572a` 上，使用真实 `file://` Git transport、depth=1/2、旧 detached HEAD 和已前进的 origin/main，复现替代目录 checkout 的 `unable to read tree`。浅镜像的本地 clone 不保证带上仅由原目录 remote-tracking ref 指向的新目标对象；旧完整仓库用例没有覆盖此形态。

最小修复在 clone 后，从原镜像按已经冻结的 `target` SHA 显式 fetch 到替代目录，再做既有 checkout 和交换。不额外访问远端、不解除 shallow、不改原目录的代码或本地状态。取对象失败仍在任何目录移动之前退出；只允许两个生成文件、固定保留目录、未知改动拒绝和交换失败恢复规则全部保持。

新增真实 depth=1/2 回归验证旧 HEAD、两份生成物和未跟踪状态完整保留，新镜像干净且到冻结目标；另注入仅作用于替代目录的 fetch 失败，验证原目录保留、临时目录和锁清理、其他策略仓继续处理。此前 full clone、重复运行、未知修改、交换失败及安装隔离用例继续保留。源码测试不代表已重新部署或恢复定时器；实际 VPS 状态由部署方单独验收。
