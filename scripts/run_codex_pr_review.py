...
REVIEW_COMPLETED_MARKER_PREFIX = "<!-- codex-pr-review-completed:"
...
7. Review only the supplied current exact-head diff. Do not inherit findings, fingerprints, retry counts, or arbitration state from any prior head.
...
def build_pr_comment(
    ...
    *,
    ...
    """Build a comment containing only the current exact-head result."""
    lines = [
        "<!-- codex-pr-review -->",
        f"{HEAD_SHA_MARKER_PREFIX}{reviewed_head_sha}{HEAD_SHA_MARKER_SUFFIX}",
        f"{REVIEW_COMPLETED_MARKER_PREFIX}{str(review_completed).lower()}{DECISION_MARKER_SUFFIX}",
...
def write_decision_outputs(decision_payload: dict[str, Any]) -> None:
    ...
    if github_output:
        with open(github_output, "a", encoding="utf-8") as f:
            ...
            f.write(
                f"review_completed={'true' if decision_payload['review_completed'] else 'false'}\n"
            )
...
def publish_review_decision(
    ...
    *,
    ...
    """Publish one current-head comment, artifact, and step-output decision."""
    upsert_pr_comment(
        token,
        repo,
        pr_number,
    ...
    write_decision_outputs(
        {
            **decision,
            "review_completed": review_completed,
            "reviewed_head_sha": current_head_sha if review_completed else "",
            "current_head_sha": current_head_sha,
            "review_implementation_digest": review_implementation_digest(),
        }
    )
    return exit_code


def publish_retryable_review_failure(
    token: str,
    repo: str,
    pr_number: int,
    pr_url: str,
    ...
    """Record an external retryable failure without marking the head reviewed."""
    return publish_review_decision(
        ...
        pr_url,
        {
            ...
            "total_findings": 0,
            "summary": (
                "⚠️ **Review incomplete**: external retryable failure; "
                "this head has not been successfully reviewed."
            ),
            "failure_kind": "external_retryable",
        },
        exit_code=1,
        current_head_sha=current_head_sha,
...
def main() -> int:
    ...
    if not re.fullmatch(r"[0-9a-f]{7,64}", current_head_sha):
        print("::error::Pull request exact head SHA is missing or invalid", file=sys.stderr)
        return 1
    ...
    # Each invocation reviews only the event's exact head. Prior PR comments are
    # presentation records, never input to the current application decision.
    ...
    try:
        complexity = _estimate_review_complexity(
            diff, changed_paths, title=pr_title, body=pr_body
        )
        ...
        print(f"::warning::Codex review external retryable failure: {exc}")
        return publish_retryable_review_failure(
            token, repo, pr_number, pr_url, current_head_sha
        )
    ...
    try:
        ...
        print(f"::warning::Codex review parse failure is retryable: {exc}")
        return publish_retryable_review_failure(
            token, repo, pr_number, pr_url, current_head_sha
        )
    ...
    if decision["blocked"]:
        print("::error::Merge blocked: serious issues found on current exact head")
    else:
        print("Review passed: no blocking issues on current exact head")
    return publish_review_decision(
        token,
        repo,
        pr_number,
