...
from service.model_catalog_sync import sync_catalog, sync_subscription_catalog  # noqa: E402
...
def main() -> int:
    ...
    parser.add_argument("--subscriptions-only", action="store_true", help="Refresh target account CLI roster without API discovery.")
    parser.add_argument("--json", action="store_true", help="Print resulting catalog JSON.")
    args = parser.parse_args()

    if args.subscriptions_only:
        catalog = sync_subscription_catalog(output_path=str(args.output) if args.output else None)
    else:
        catalog = sync_catalog(output_path=str(args.output) if args.output else None, force=bool(args.force))
    if args.json:
        ...
        print(
            json.dumps(
                {
                    "subscription_rosters": catalog.subscription_rosters,
