...
_GLOBAL_ETF_RESEARCH_CODEGEN_TASK = "global_etf_research_codegen"
...
def resolve_codex_research_route(
    *, stage: str, account: dict | None, now: float,
    ...
    global_codegen = (
        str(task or "").strip() == _GLOBAL_ETF_RESEARCH_CODEGEN_TASK
        and stage == "optimization"
    )
    bounded_codegen = bounded_codegen or global_codegen
    bounded_reason = "global_codegen_route_invalid" if global_codegen else "soxl_codegen_route_invalid"
    level = 1 if bounded_codegen else max(_CODEX_RESEARCH_LEVELS[stage], {"low": 0, "medium": 1, "high": 2}[complexity])
    if bounded_codegen and requested_model not in {"", "auto", "gpt-5.6-luna"}:
        return {**result, "research_stage": stage, "reason": bounded_reason}
    if bounded_codegen and requested_effort not in {"", "auto", "medium"}:
        return {**result, "research_stage": stage, "reason": bounded_reason}
    model = requested_model if requested_model not in {"", "auto"} else (
        "gpt-5.6-luna" if bounded_codegen else _CODEX_RESEARCH_MODELS[level]
    )
    effort = requested_effort if requested_effort not in {"", "auto"} else (
        "medium" if bounded_codegen else _CODEX_RESEARCH_EFFORTS[level]
    )
    ...
    if model_levels[model] < level and not (bounded_codegen and model == "gpt-5.6-luna"):
